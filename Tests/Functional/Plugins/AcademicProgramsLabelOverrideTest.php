 ...
 * passes, only lowercased, so a name with underscores read `plugin.tx_academic_programs`
 ...
 * only as the first translation of the file: the plugins hide the category filter, whose
 * partial translates correctly since ACE-739, and the sorting option is rendered from a
 * partial of this test that holds the select alone, so the options are read with the
 * default name of the view helper. After the first translation of a file, the check of the
 * extension names of all translations holds a call: TYPO3 v14 strips the underscores and
 * falls back to the name of the plugin request, so a lost or underscored name reads the
 * same paths there, and no case could catch it either.
 *
 ...
 * its page type, whose header links back to the list.
 */
final class AcademicProgramsLabelOverrideTest extends AbstractAcademicProgramsTestCase
{
     ...
     * plugin, so only the extension path applies. Its header links back to the list.
     */
    private function renderProgramPage(string $setup): string
    {
        ...
        $connection->update(
            'sys_template',
            ['constants' => $template['constants'] . LF . 'plugin.tx_academicprograms.page.listPid = 2'],
            ['uid' => $template['uid']],
        );
    ...
    /**
     * Every kind of translation the list renders: a label of a partial, one whose key comes
     * from PHP, and the options of the sorting select, which its view helper translates in
     * PHP with a full `LLL:` reference.
     *
    ...
    /**
     * The page template translates the link back to the list in its header, and the facts
     * of the program, whose keys come from PHP: a category type and a program field.
     *
